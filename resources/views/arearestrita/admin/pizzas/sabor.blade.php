...
<div id="modal_delete" class="modal" style= "width: 35%; heighth: 20%;">
  <div class="modal-content  grey lighten-4">
    <form action="{{route('admin.deletar.sabor')}}" method="POST" class="remove-record-model">
      {{ method_field('delete') }}
      {{ csrf_field() }}
    <h4 class="center-align">Você deseja deletar este sabor?</h4>
    <p class="center-align red-text">SE HOUVER PIZZAS COM ESTE SABOR, AS MESMAS SERÃO DELETADAS.</p>
  </div>
  <div class="modal-footer  grey lighten-2">
    <input type="hidden" name="id_sabor" id="pizza_id">
...
<div class="fixed-action-btn">
  <a class="btn-floating btn-large green" href="{{route('admin.criar.sabor')}}">
...
<tbody>
 ...
 <tr>
  <td>{{$value->sabor}}</td>
  <td>{{$value->desc_sabor}}</td>
  <td>{{$value->created_at}}</td>   
  <td> <button class='waves-effect red waves-light btn' href="#modal_delete" id="delete-id" data-sabor-id="{{$value->id_sabor}}"><i class="material-icons">delete</i></button></td>
</tr>
  @endforeach 
</tbody>
</table>
